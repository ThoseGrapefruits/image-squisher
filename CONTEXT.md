...
Lossless-capable image conversion tool. Scans a folder, converts each image to JPEG XL, WebP, and progressive JPEG, and keeps the original plus every successful conversion beside it. Defaults are visually lossless (quality 90), which is appropriate for camera HEIC/JPEG. Set `jpegxl_quality` to 100 and `webp_lossless` to true for mathematically lossless output. Progressive JPEG is always a Pillow re-encode.
...
processor.py                 JPEG XL (cjxl), WebP (Pillow), progressive JPEG (Pillow)
...
2. `format_detector.scan_folder` finds files. `--source` / `source_extensions` limits by extension; otherwise all known image types minus `skip_extensions` (default `.webp`, `.jxl`). Names ending in `.p.jpg` are always skipped, including when `--source .jpg` is set. Headers are checked with Pillow; pixels are not decoded at scan time.
3. Worker threads (or one thread) call `file_manager.process_image` per file.
4. `processor.convert_image` writes `{stem}.tmp.jxl`, `{stem}.tmp.webp`, and `{stem}.tmp.p.jpg` in the image directory. JXL and WebP are skipped when the source is already that format. Progressive JPEG is skipped for animated sources and for files already named `{stem}.p.jpg`.
5. `file_manager.save_converted_file` atomically moves temps to `{stem}.jxl`, `{stem}.webp`, and `{stem}.p.jpg`. Same-stem inputs in one folder share those output names.
...
| Progressive JPEG | Pillow `Image.save(format='JPEG', progressive=True)` | `{stem}.p.jpg`. Quality `jpeg_quality` (default 90), `optimize=True`. Alpha is composited onto white. ICC profile is kept. Animated sources are skipped. |
...
| `jpeg_quality` | 90 | 1–100. Progressive JPEG `{stem}.p.jpg`. Re-encode, so a source JPEG is a second lossy generation. |
...
- `photo.p.jpg` (if progressive JPEG succeeded)

`photo.jpg` and `photo.png` both write `photo.jxl` / `photo.webp` / `photo.p.jpg`; the later file wins. Re-runs overwrite those outputs. `photo.p.jpg` is not scanned as a source, so it is not encoded again into `photo.p.p.jpg`.
